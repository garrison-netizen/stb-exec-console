...
import { loadTaproom, loadSkuMonths, CLOVER_FROM, ORDERS_ERA_FROM, provenanceForDate } from './taproomCore.js'
...
// What the assistant must say whenever Arryved-era rows are in scope. Two
// distinct traps: the era's missing columns are NULL rather than zero, and its
// comps are not lost revenue — private-event open-bar drinks were rung at the
// register to track pour, then comped to $0, because that revenue was billed
// on the Private Events side. Counting comps as taproom revenue double-counts
// against PE.
const ARRYVED_CAVEAT =
  'Arryved-era rows carry net revenue, tax and comps ONLY — transactions, gross, tips, ' +
  'tender and discounts are NULL (no data), never zero, so do not report them as $0 or ' +
  'average them away; comps are private-event pour billed on the Events side, not taproom revenue'
...
export async function getTaproomDb() {
  ...
  db.run(`
    CREATE TABLE daily (
      ...
      discounts REAL, comps REAL, tender_card REAL, tender_cash REAL, tender_other REAL,
  ...
  // MECHANICAL GUARD, not prose discipline: the Arryved era simply has no
  // transaction, gross, tip, tender or discount data, so those cells go in as
  // real NULLs. SQLite's AVG/SUM then skip them by construction — a question
  // about 2022 tips returns NULL ("no data"), never a confident $0. Zeroes
  // here would be indistinguishable from a genuine zero-tip day, and the
  // assistant would have no way to tell the difference.
  const arryvedNull = (d, x) => (provenanceForDate(d.date) === 'arryved' ? null : v(x))
  // Discounts need a WIDER guard than the rest: they are unknowable in the
  // Arryved era AND in the payments-rebuild era, because payment records carry
  // no discount data. Only the orders era can answer. Without this, a question
  // like "how much did we discount last year" sums a wall of zeroes and
  // answers $0 with total confidence.
  const discountNull = (d, x) => (provenanceForDate(d.date) === 'orders' ? v(x) : null)
  const dailyRow = (d) => [
    v(d.date), v(d.dow), arryvedNull(d, d.transactions), arryvedNull(d, d.gross),
    v(d.net), v(d.tax), arryvedNull(d, d.tips), discountNull(d, d.discounts), v(d.comps),
    arryvedNull(d, d.card), arryvedNull(d, d.cash), arryvedNull(d, d.other),
    v(d.source), provenanceForDate(d.date),
  ]
...
export function provenanceStamp(sql, rows) {
  ...
  if (Array.isArray(rows) && rows.length && 'provenance' in rows[0]) {
    ...
    const caveats = []
    if (rebuilt) caveats.push('discounts/refunds')
    if (counts['arryved']) caveats.push(ARRYVED_CAVEAT)
    return `PROVENANCE: rows returned — ${parts.join(', ')}. ` +
      (caveats.length ? `CAVEAT REQUIRED in your answer (${caveats.join('; ')}).` : 'No caveat needed.')
  }
  ...
  const before = (d, boundary) => (d.length === 7 ? d < boundary.slice(0, 7) : d < boundary)
  const beforeBoundary = literals.filter((d) => before(d, ORDERS_ERA_FROM))
  const reachesArryved = literals.filter((d) => before(d, CLOVER_FROM))
  if (!literals.length) {
    return `PROVENANCE: unfiltered query over daily — spans all three eras (arryved before ${CLOVER_FROM}, ` +
      `payments-rebuild before ${ORDERS_ERA_FROM}, orders after). ` +
      `CAVEAT REQUIRED in your answer (discounts/refunds; ${ARRYVED_CAVEAT}).`
  }
  if (reachesArryved.length) {
    return `PROVENANCE: this query's date range reaches before ${CLOVER_FROM}, so it includes Arryved-era rows. ` +
      `CAVEAT REQUIRED in your answer (${ARRYVED_CAVEAT}).`
  }
...
for the taproom's register data, answering the taproom team (and executives)
...
TABLE daily — one row per open day, ${coverage?.from || '2025-03-06'} → ${coverage?.to || 'yesterday'}.
  ...
  tax_collected, tips, discounts, comps, tender_card, tender_cash, tender_other,
  source ('arryved' | 'clover').
  TWO POS ERAS. Before ${CLOVER_FROM} the taproom ran on Arryved, whose exports carry
  net_revenue, tax_collected and comps ONLY — transactions, gross_revenue, tips,
  discounts and all tender columns are NULL for those rows. They are NULL because the
  data does not exist, NOT because the day was zero. SQLite already skips NULLs in
  SUM/AVG, so a whole-history "average tips" is silently a Clover-era figure: say so.
  If a question needs a NULL column, scope it to source='clover' and state the window.
  net_revenue and tax_collected are the only money columns safe to trend across eras.
  DISCOUNTS ARE NARROWER STILL. discounts is NULL for BOTH the Arryved era and the
  payments-rebuild era (${CLOVER_FROM} → ${ORDERS_ERA_FROM}) — payment records carry no
  discount data at all. Only rows from ${ORDERS_ERA_FROM} forward have real discounts.
  So a discount total covering anything earlier is NOT measured, and you must say so
  rather than report the orders-era figure as if it were the whole period. Item-level
  discount history for 2025-03 → 2026-04 exists only in sku_months' source exports and
  is not in this database; if asked for it, say where it lives instead of guessing.
...
Every table has a provenance column ('arryved' | 'payments-rebuild' | 'orders' |
'export-load'), and every query result is prefixed with a PROVENANCE: line stating
whether a caveat is required. Follow that line literally:
- "CAVEAT REQUIRED" → your answer MUST state, unprompted, exactly the caveat the
  stamp names. For rebuilt payment records: discounts are NULL because payments carry
  no discount data (they are NOT zero, and were wrongly stored as 0 until 2026-07-31),
  and days with refunds read slightly high. For Arryved-era rows: the missing columns are
  no-data rather than zero, and comps are private-event open-bar pour billed on the
  Events side — never count comps as taproom revenue, it double-counts against PE.
...
(For reference only: arryved covers dates before ${CLOVER_FROM}, payments-rebuild
covers ${CLOVER_FROM} to ${ORDERS_ERA_FROM}.)
