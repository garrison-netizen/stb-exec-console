...
export async function getTaproomDb() {
  ...
  const dailyRow = (d) => [
    ...
    v(d.net), v(d.tax), arryvedNull(d, d.tips), arryvedNull(d, d.discounts), v(d.comps),
...
- "CAVEAT REQUIRED" → your answer MUST state, unprompted, exactly the caveat the
  stamp names. For rebuilt payment records: discounts show as 0 (not truly zero) and
  days with refunds read slightly high. For Arryved-era rows: the missing columns are
