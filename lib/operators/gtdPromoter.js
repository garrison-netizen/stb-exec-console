...
//
// Draft-hold mechanism (proposed at build time per ADR-007 §6): Labels
// multi-select. Every Task lands with Label "🤖 Operator draft" plus
// "Domain: <X>" tag. Garrison releases by removing the draft label.
//
...
const DRAFT_HOLD_LABEL = '🤖 Operator draft';
...
function buildDescription(row) {
  const body = row.body || '';
  const truncated = body.length > BODY_TRUNCATE
    ? body.slice(0, BODY_TRUNCATE - 3) + '...'
    : body;
  return truncated + reversePointer(row);
}
