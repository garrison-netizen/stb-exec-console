...
export async function financesDashboard() {
  ...
  const monthly = Array.from({ length: 12 }, (_, i) => {
    ...
    return { month: mm, wholesale, events, total: wholesale + events, lastYear }
  })
  ...
  const eventsLYtd = Math.round(live
    ...
    .reduce((n, b) => n + eventRev(b), 0))
  const totalYTD = wholesaleYTD + eventsYTD
  ...
  return {
    ...
    kpis: {
      totalYTD,
      totalLYtd,
      vsLYPct: totalLYtd > 0 ? Math.round((100 * (totalYTD - totalLYtd)) / totalLYtd) : null,
      wholesaleYTD,
      eventsYTD,
