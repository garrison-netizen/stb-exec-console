...
export async function financesDashboard() {
  ...
  const monthly = Array.from({ length: 12 }, (_, i) => {
    ...
    const taproomLY = taproom.byMonth[`${year - 1}-${mm}`] || 0
    const lastYear = pick(year - 1) + eventsLY + taproomLY
    return {
      month: mm, wholesale, events, taproom: taproomNet,
      total: wholesale + events + taproomNet, lastYear,
      lyHasTaproom: taproomLY > 0,
    }
  })
