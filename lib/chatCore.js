...
export async function handleChat(body, email) {
  ...
  return runChat({
    ...
    email,
    db,
    system: buildSystemPrompt(syncedAt),
    toolDescription:
      'Run one read-only SQLite SELECT against the Spindletap Ekos mirror. Returns rows as JSON. Prefer aggregated queries with LIMIT.',
    dataAsOf: syncedAt,
    label: 'chat',
...
export async function runChat({ body, email, db, system, toolDescription, dataAsOf, label = 'chat' }) {
  ...
  const tools = [{ ...TOOLS[0], description: toolDescription || TOOLS[0].description }]
