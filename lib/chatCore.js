...
import { logTurn } from './assistantLog.js'
...
// annotate: optional (sql, rows) -> string prepended to every query result —
// use it to put mandatory caveat triggers INSIDE the tool output the model
// must read, rather than relying on it to remember a rule.
export async function runChat({ body, email, db, system, toolDescription, dataAsOf, label = 'chat', space = null, extraTools = [], annotate = null }) {
  ...
  // Query-log identity: the client sends a conversation id so the audit can
  // spot a user re-asking the same thing (the strongest "that answer missed"
  // signal there is). Turn number = how many questions deep this exchange is.
  const logMeta = {
    space: space || label,
    email,
    conversationId: (body && body.conversationId) || null,
  ...
  // Struggle signals for the query log, gathered as we go.
  let rounds = 0
  const queries = []
  const sqlErrors = []
  const toolErrors = []
  ...
  try {
    ...
    logTurn({ ...logMeta, at: new Date().toISOString(), ms: Date.now() - startedAt, error: (err && err.message) || String(err) })
    err.logged = true
    throw err
  }
    ...
    for (const block of response.content) {
      ...
      try {
        if (block.name === 'query') {
          ...
          const { payload, note, rows } = runQuery(db, sql)
          queries.push(sql)
          let stamp = ''
          if (annotate) {
            try { stamp = annotate(sql, rows) || '' } catch { stamp = '' }
          }
          resultText = (stamp ? stamp + '\n' : '') + (note ? note + '\n' : '') + (payload || '[]')
        ...
        if (block.name === 'query') {
          const sql = (block.input && block.input.sql) || ''
          queries.push(sql)
          sqlErrors.push(msg + ' || ' + String(sql).slice(0, 400))
        } else {
          toolErrors.push(block.name + ': ' + msg)
        }
        resultText = ((block.name === 'query' ? 'SQL' : block.name) + ' error: ') + msg
      }
  ...
  logTurn({
    ...
    toolErrors,
    outOfTime: landedEarly,
    roundCap: rounds >= MAX_TOOL_ROUNDS,
    reply,
  })
  return {
    ok: true,
