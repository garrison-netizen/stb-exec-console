...
import { coffeeRegisterSales } from './taproomCore.js'
...
export async function coffeeDashboard() {
  ...
  // Register coffee sales (Clover SKU weeks, Category = Coffee) — best-effort:
  // absent until the Clover DBs are shared with the Console integration.
  let register = null
  try {
    register = await coffeeRegisterSales()
  } catch (err) {
    console.error('[coffee] register sales unavailable:', (err && err.message) || String(err))
  }
