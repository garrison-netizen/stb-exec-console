...
import { quoteEvent, FACILITY_TIERS, BEVERAGE_RATES, FACILITY_MINIMUMS } from './eventQuote.js'
...
- deposit_paid / balance_paid / deposit_amount are NOT SYNCED (Triple Seat's
  bookings feed carries no payment data; the payments pipeline is pending, so
  these columns sit at defaults). NEVER answer payment-status questions from
  them — say payment data isn't connected yet and, if useful, note the Q2
  close reconciles payments monthly in QBO.
...
PRICING (the live Private Event Calculator's logic — you have it as the "quote"
tool): facilities = ${Object.keys(FACILITY_TIERS).join(', ')}; beverage packages =
${Object.keys(BEVERAGE_RATES).map((b) => `${b} ($${BEVERAGE_RATES[b]}/21+/hr)`).join(', ')};
facility minimums = ${Object.entries(FACILITY_MINIMUMS).map(([f, m]) => `${f} $${m.toLocaleString('en-US')}`).join(', ')}.
Facility pricing is tiered per person per hour (marginal bands), SpindlePark is a
SpindleBarn-only add-on, off-peak takes 10% off the facility portion and the
minimum, 2-hour rental minimum. For ANY price/quote question, call the quote
tool — NEVER compute tier math yourself. Quotes are estimates in the calculator's
terms; the booked contract governs.`
}
...
const QUOTE_TOOL = {
  definition: {
    name: 'quote',
    description:
      'Compute a private-event quote with the exact pricing logic of the live Private Event Calculator. Use for any pricing/quote/what-would-it-cost question.',
    input_schema: {
      type: 'object',
      properties: {
        facility: { type: 'string', enum: Object.keys(FACILITY_TIERS) },
        beverage: { type: 'string', enum: Object.keys(BEVERAGE_RATES) },
        attendees: { type: 'number' },
        attendeesOver21: { type: 'number' },
        hours: { type: 'number' },
        spindlePark: { type: 'boolean', description: 'SpindleBarn only' },
        offPeak: { type: 'boolean' },
      },
      required: ['facility', 'beverage', 'attendees', 'hours'],
    },
  },
  handler: (input) => JSON.stringify(quoteEvent(input)),
}
