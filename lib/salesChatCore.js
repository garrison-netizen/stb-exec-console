...
export async function getSalesDb() {
  ...
  // The newest week in Mart C is partial at pull time (restated next pull) —
  // flag it so the assistant can exclude it from week-over-week comparisons.
  const provisionalWeek = (martC && martC.length)
    ? martC.map((r) => r.week).sort().slice(-1)[0]
    : null
...
rolling ~13-week trailing window that accumulates over time).
  ...
  provisional (0/1).
  CRITICAL: rows with provisional=1 are the NEWEST week, which is PARTIAL —
  VIP's export caught it mid-week; next week's pull restates it to full. For
  any week-over-week trend, momentum, or "latest week" question, EXCLUDE
  provisional=1 (WHERE provisional=0) unless the user explicitly asks about the
  in-progress week — and if you include it, say it is partial. There is no
  year-over-year at weekly grain; use the depletions/accounts tables for YoY.
  Use this table for recent momentum ("which brands are moving lately",
  "last few weeks"), NOT for annual totals.
