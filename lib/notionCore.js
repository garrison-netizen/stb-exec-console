...
export async function dispatchSubmission(submission) {
  ...
  switch (submissionType) {
    ...
    case 'create_task':
      return await createTask(submission);
    case 'update_task':
      return await updateTask(submission);
    default:
      throw new Error(`Unknown submissionType: ${submissionType}`);
  }
}

// Console add/edit (2026-07-10, Garrison directive): the Console is the
// working surface, Notion the backend. Create + field-level update on UB
// Tasks. Still NEVER the Brain — this is Personal Dashboard scope only.
export async function createTask({ name, domain = null, due = null, priority = null, projectId = null, description = null }) {
  const dsId = process.env.NOTION_UB_TASKS_DS;
  if (!dsId) throw new Error('NOTION_UB_TASKS_DS not set');
  ...
  if (domain) properties.Labels = { multi_select: [{ name: `Domain: ${domain}` }] };
...
// Field-level task update. Only keys present in the submission are written;
// an explicit null clears that field (Due, Priority, Project, Domain).
// Domain swaps the 'Domain: X' label while preserving any other labels.
export async function updateTask(fields) {
  const { pageId } = fields;
  if (!pageId) throw new Error('pageId is required');
  ...
  if ('priority' in fields) {
    properties.Priority = fields.priority ? { status: { name: fields.priority } } : { status: null };
  }
  if ('taskStatus' in fields && fields.taskStatus) {
    properties.Status = { status: { name: fields.taskStatus } };
  }
  if ('projectId' in fields) {
    properties.Project = { relation: fields.projectId ? [{ id: fields.projectId }] : [] };
  }
  if ('domain' in fields) {
    const page = await client().pages.retrieve({ page_id: pageId });
    const others = (page.properties?.Labels?.multi_select || [])
      .map((s) => s.name)
      .filter((n) => !n.startsWith('Domain: '));
    const next = fields.domain ? [...others, `Domain: ${fields.domain}`] : others;
    properties.Labels = { multi_select: next.map((name) => ({ name })) };
  }
  if (!Object.keys(properties).length) throw new Error('no fields to update');

  await client().pages.update({ page_id: pageId, properties });
  return { pageId, updated: Object.keys(properties) };
}
