...
const SPACE_TAGS = ['Exec', 'Finances', 'Production', 'Events', 'Taproom', 'Sales', 'Marketing', 'Coffee', 'R&D']

// Annexed standalone apps (same allow-list, own Vercel deployments). A user
// sees the nav link only when they hold the tag; Exec sees both.
const APP_TAGS = ['Calendar', 'Calculator']
...
function appsFor(tags) {
  if (tags.has('Exec')) return APP_TAGS.slice()
  return APP_TAGS.filter((a) => tags.has(a))
}
...
export async function requireAuth(req) {
  ...
  if (!clientId) {
    ...
    return { email: null, spaces: SPACE_TAGS.slice(), apps: APP_TAGS.slice() } // local dev — no gate
  }
  ...
  const apps = appsFor(tags)
  if (spaces.length === 0 && apps.length === 0)
    throw fail('Your account has no app spaces yet. Contact Garrison for access.', 403)

  return { email, spaces, apps }
}
