...
// the Brain (ADR-013). Read-only.
//   Mart A — Depletion Trend: Brand × Distributor × Segment × Year, CE etc.
//   Mart B — Account Trajectory: one row per account; CE history columns are
...
export async function loadMarts({ force = false } = {}) {
  ...
  cache = { at: Date.now(), data: { martA, accounts, cols, loadedAt: new Date().toISOString() } }
  return cache.data
}
...
export async function salesDashboard({ force = false } = {}) {
  const { martA, accounts, cols, loadedAt } = await loadMarts({ force })
