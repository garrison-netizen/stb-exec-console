...
// The newest N weeks of every VIP pull are still SETTLING: week N is partial
// (the export catches it mid-week) and week N-1 is typically still being
// restated upward as distributors report with a lag. Both are restated by
// later overlapping pulls (accumulating, last-write-wins). Garrison's call
// (2026-07-23): SHOW each week's actual number, but flag the newest two as
// settling and EXCLUDE them from trend/momentum so a still-filling week never
// reads as a real decline.
const SETTLING_WEEKS = 2

// Weekly model from Mart C.
function weeklyModel(martC) {
  ...
  const settlingFrom = Math.max(0, weeks.length - SETTLING_WEEKS)
  const settledWeeks = weeks.slice(0, settlingFrom)
  const settlingWeeks = weeks.slice(settlingFrom)
  const settlingSet = new Set(settlingWeeks)

  const labelByWeek = new Map(martC.map((r) => [r.week, r.weekLabel]))
  const ceByWeek = sumBy(martC, (r) => r.week, (r) => r.ce)
  const entry = (w) => ({
    week: w,
    label: labelByWeek.get(w) || w,
    ce: r1(ceByWeek.get(w)),
    settling: settlingSet.has(w),
  })

  // Every week, actual number shown; settling ones flagged. Newest first.
  const series = [...weeks].reverse().map(entry)

  // Brand momentum over SETTLED weeks only: latest settled week vs the mean of
  // up to 4 settled weeks before it.
  let momentum = null
  if (settledWeeks.length >= 2) {
    const latest = settledWeeks[settledWeeks.length - 1]
    const base = settledWeeks.slice(-5, -1) // up to 4 prior settled weeks
  ...
  // Trend window covers the SETTLED weeks (what momentum/trend is measured on).
  const first = settledWeeks[0], last = settledWeeks[settledWeeks.length - 1]
  const windowLabel = settledWeeks.length
    ? `${(labelByWeek.get(first) || first).split(' thru ')[0]} thru ${(labelByWeek.get(last) || last).split(' thru ')[1] || last}`
    : null

  return {
    series,                 // ALL weeks (newest first), each tagged .settling
    settledCount: settledWeeks.length,
    settlingWeeks: settlingWeeks.map(entry),  // the newest 2 — shown but flagged
    windowLabel,            // covers settled weeks (the trend window)
    momentum,
  }
}
