...
// Trim + strip wrapping quotes: env values pasted or piped in can carry a
// stray CR/newline/quote that Notion rejects as an invalid uuid.
const cleanEnv = (v) => (v || '').trim().replace(/^["']|["']$/g, '').trim();

const NOTION_TOKEN = cleanEnv(process.env.NOTION_TOKEN);
const LEADS_DS = cleanEnv(process.env.NOTION_TS_LEADS_DS);
const BOOKINGS_DS = cleanEnv(process.env.NOTION_TS_BOOKINGS_DS);
const SOURCES_DS = cleanEnv(process.env.NOTION_TS_SOURCES_DS);
...
const L = {
  ...
  HEADCOUNT: 'Headcount estimate', CONVERTED_AT: 'Converted at',
};
const B = {
...
export async function loadNormalized() {
  ...
  const leads = leadRows.map((r) => ({
    ...
    convertedAt: dateISO(r, L.CONVERTED_AT),
    headcount: num(r, L.HEADCOUNT),
...
export function computeMetrics(leads, bookings, today = todayCT()) {
  ...
  const unpaidBalances = live
    ...
    .sort((a, b) => rev(b) - rev(a)); // biggest dollars first — the list is capped
  // Only revenue-carrying events inside the upcoming window: $0 community
  // events and far-future bookings aren't actionable deposit chases.
  const unpaidDeposits = live
    .filter((b) => b.eventDate > today && b.eventDate <= hUp && !b.depositPaid && rev(b) > 0)
    .sort((a, b) => (a.eventDate < b.eventDate ? -1 : 1));
  ...
  const bookedForward = live
    .filter((b) => b.eventDate > today)
    .reduce((n, b) => n + rev(b), 0)
  const bookedForwardCount = live.filter((b) => b.eventDate > today).length

  return {
    generatedAt: new Date().toISOString(),
    today,
    // Deposit/balance checkboxes are NOT synced from Triple Seat (the
    // bookings endpoint doesn't carry payment status — it lives on the
    // Events/Payments endpoints, pending the payments pipeline). The UI
    // hides all payment-status surfaces until this flips true.
    paymentsSynced: false,
    kpis: {
      ytdRevenue, ytdEvents, ytdLastYear,
      next30Count: next30.length,
      next30Revenue: next30.reduce((n, b) => n + rev(b), 0),
      bookedForward, bookedForwardCount,
      openLeads,
    },
    monthly,
    upcoming,
    attention: {
      unpaidBalances: unpaidBalances.slice(0, 12).map((b) => ({
        date: b.eventDate, title: b.title, revenue: rev(b), depositPaid: b.depositPaid, rep: b.rep,
      })),
      unpaidBalanceCount: unpaidBalances.length,
      unpaidBalanceTotal: unpaidBalances.reduce((n, b) => n + rev(b), 0),
      unpaidDeposits: unpaidDeposits.slice(0, 10).map((b) => ({
